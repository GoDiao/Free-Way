...
- **一个本地入口接入 AI 工具**：Claude Code、Continue.dev、OpenCode 或任何能从本机直接请求自定义 base URL 的兼容客户端，都可以指向同一个网关
...
- **支持 Claude Code、Continue.dev、OpenCode，以及能从本机直接请求自定义 base URL 的 OpenAI/Anthropic 兼容客户端**
...
Cursor 可能无法直接使用本机的 Free-Way endpoint。部分 Cursor API 流程会先经过 Cursor 自己的服务器，再调用你配置的 API；在这种情况下，`localhost:8787` 指向的是 Cursor 服务器环境，而不是你的电脑。

Free-Way 更适合能从本机直接请求自定义 OpenAI/Anthropic 兼容 base URL 的客户端。
